...
pub fn extract_pak_to_dir(pak: &InstallableMod, install_dir: PathBuf) -> Result<(), String> {
    ...
    if let Some(config_dir) = dirs::config_dir() {
        let debug_log = config_dir.join("BentoMod").join("extract_pak_debug.log");
...
pub fn create_bento_from_pak_fast(
    pak: &InstallableMod,
    mod_dir: PathBuf,
    packed_files_count: &AtomicI32,
) -> Result<(), String> {
    ...
    info!(
        "[CreateBentoFast] Creating IoStore directly from PAK: {}",
        pak.mod_path.display()
    );
    info!("[CreateBentoFast] Output base: {}", output_base.display());
    ...
    info!(
        "[CreateBentoFast] IoStore created: utoc={}, converted={} files",
