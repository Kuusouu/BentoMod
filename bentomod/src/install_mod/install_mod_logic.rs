...
/// Clean up any existing variants of a mod file (.bak_bento, .pak_disabled) before installing
...
fn cleanup_existing_mod_variants(output_dir: &Path, base_name: &str) {
    ...
    for ext in &iostore_extensions {
        let variants = [
            ...
            format!("{}.{}.bak_bento", base_name, ext),
...
pub fn install_mods_in_viewport(
    ...
    for installable_mod in mods.iter_mut() {
        ...
        crate::install_mod::write_install_debug(&format!(
            "=== Installing mod: name={}, iostore={}, bento={}, is_dir={}, mod_path={}, mod_path_exists={}",
            installable_mod.mod_name, installable_mod.iostore, installable_mod.bento, 
        ...
        if installable_mod.bento {
            crate::install_mod::write_install_debug(
                "  -> Taking BENTO path (direct PAK -> IoStore)",
            );
            // Clean up any existing variants before installing
            let base = normalize_mod_base_name(&installable_mod.mod_name, 7);
            cleanup_existing_mod_variants(&output_directory, &base);
            ...
            match pak_files::create_bento_from_pak_fast(
                installable_mod,
                output_directory.clone(),
                installed_mods_ptr,
            ) {
                Err(e) => {
                    error!("Failed to create bento from pak: {}", e);
                    crate::install_mod::write_install_debug(&format!("  ERROR bento: {}", e));
                    results.push(ModInstallResult {
                        mod_name: installable_mod.mod_name.clone(),
                        success: false,
                        error: Some(e),
        ...
        if !installable_mod.bento && !installable_mod.is_dir {
        ...
        if installable_mod.is_dir {
            ...
            match res {
                Err(e) => {
                    error!("Failed to create bento from directory: {}", e);
                    results.push(ModInstallResult {
                        mod_name: installable_mod.mod_name.clone(),
                        success: false,
                        error: Some(e),
