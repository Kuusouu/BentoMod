...
pub fn contains_raw_files(files: &[String]) -> bool {
    files.iter().any(|f| {
        ...
        // Ignore specific bento/iostore metadata files and directories
        ...
        if file_path.is_file() && file_path.extension().and_then(|s| s.to_str()) == Some("pak") {
            ...
            if utoc_path.exists() && ucas_path.exists() {
                ...
                let installable_mod = InstallableMod {
                    ...
                    bento: false, // Don't use bento workflow for iostore mods
                    is_dir: false,
                    reader: None,
            ...
            // This is a standalone .pak file
                ...
                write_install_debug("  -> Standalone PAK path (bento)");
                ...
                write_install_debug(&format!(
                    "  Created InstallableMod: name={}, bento={}, iostore={}, type={}, files={}",
                    installable_mod.mod_name,
                    installable_mod.bento,
                ...
                if entry_path.is_dir() {
                    ...
                    if has_content {
                        ...
                        let installable_mod = InstallableMod {
                            mod_name,
                            mod_type: modtype.to_string(),
                            bento: !is_audio_or_movies, // Will go through convert_to_iostore_directory
...
pub fn write_install_debug(msg: &str) {
    if let Some(config_dir) = dirs::config_dir() {
        let debug_log = config_dir.join("BentoMod").join("install_debug.log");
...
fn map_to_mods_internal(paths: &[PathBuf]) -> Vec<InstallableMod> {
    ...
    let mut installable_mods = paths
        ...
        .map(|path| {
            ...
            // Determine if we should bento this mod
            // Don't bento if: it's a directory, IoStore package, or Audio/Movies mod
            let is_audio_or_movies = modtype.contains("Audio") || modtype.contains("Movies");
            let should_bento = !is_dir && !is_iostore && !is_audio_or_movies;
