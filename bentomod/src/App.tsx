...
function App() {
  ...
  const handleViewChangelog = async () => {
    try {
      ...
      const res = await fetch('https://api.github.com/repos/Kuusouu/BentoMod/releases/latest', {
  ...
  const handleBulkToggle = async (enable: boolean) => {
    ...
    try {
      ...
      const normalizeToggleKey = (path: string) => path.replace(/\.(pak|bak_bento|pak_disabled)$/i, '')
      let count = 0
    ...
    try {
      ...
      if (lastSeen && lastSeen !== ver) {
        ...
        try {
          ...
          if (!releaseBody) {
            const latestUrl = 'https://api.github.com/repos/Kuusouu/BentoMod/releases/latest'
    ...
    try {
      ...
      if (selectedMod && selectedMod.path === modPath) {
        const updatedMod = refreshedMods.find(m =>
          m.path.replace(/\.(pak|bak_bento)$/i, '') === baseName
        )
        if (updatedMod) {
