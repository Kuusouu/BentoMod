...
const AURORA_PALETTES: Record<string, string[]> = {
	"#be1c1c": ["#be1c1c", "#ff9800", "#ffcc00", "#ff6b35"], // Bento Red: warm fire tones
...
function App() {
	...
	const [updateDownloadProgress, setUpdateDownloadProgress] =
		useState<UpdateDownloadProgress | null>(null); // { percentage, status }
	...
	const [newFolderFromInstall, setNewFolderFromInstall] =
		useState<NewFolderFromInstallState | null>(null); // { callback: (name) => void } when prompting for new folder from install panel
	const [renameFolderPrompt, setRenameFolderPrompt] = useState<RenameFolderPromptState | null>(
		null,
	); // { folderId, currentName } when prompting for folder rename
	...
	const withPromiseTransitionLoader = async <T,>(
		message: string,
		work: () => Promise<T>,
	): Promise<T> => {
	...
	const handleSendToVfxUpdater = async (mod: ModRecord | null) => {
		...
		const utocPath =
			typeof mod.utoc_path === "string" && mod.utoc_path.length > 0 ? mod.utoc_path : null;

		if (!utocPath) {
			alert.warning(
				"No UTOC Metadata",
				"Selected mod does not include backend .utoc metadata for VFX Updater.",
			);
			return;
		}
	...
	const handleFileDrop = async (paths: string[]) => {
		...
		if (targetFolder) {
			...
			try {
				const modsData = (await invoke("parse_dropped_files", {
					paths,
				})) as DroppedModParse[];
				const hasFolderWithUassets = modsData.some(
					(mod: DroppedModParse) => mod.is_dir === true && mod.contains_uassets !== false,
				);
		...
		try {
			...
			const allPaksWithNoUassets = modsData.every(
				(mod: DroppedModParse) => mod.is_dir === false && mod.contains_uassets === false,
			);
	...
	const handleMergeHybrid = async (path1: string, path2: string) => {
		try {
			const mergedModsData = (await invoke("merge_mods_for_hybrid", {
				path1,
				path2,
			})) as DroppedModParse[];
	...
	const handleCheckClashes = async () => {
		try {
			await withPromiseTransitionLoader("Checking conflicts...", async () => {
				...
				console.debug("[Conflicts] Global conflict check completed", {
	...
	const handleViewChangelog = async () => {
		try {
			...
			const res = await fetch(
				"https://api.github.com/repos/Kuusouu/BentoMod/releases/latest",
				{
					headers: { Accept: "application/vnd.github.v3+json" },
				},
			);
			console.debug("[Changelog] Latest changelog fetch response", {
				status: res.status,
				ok: res.ok,
			});
			...
			if (!releaseBody) {
				alert.info(
					"No Changelog",
					"No changelog content was found for the latest release.",
				);
				return;
			}
	...
	const handleApplyUpdate = async () => {
		...
		try {
			console.debug("[Updates] Applying update and handing off to backend auto-exit flow", {
	...
	const handleCheckSingleModClashes = async (mod: ModRecord) => {
		try {
			await withPromiseTransitionLoader("Checking conflicts...", async () => {
				console.debug("[Conflicts] Starting single-mod conflict check", {
					modPath: mod.path,
				});
				...
				const transformedClashes: ClashRecord[] = Array.from(fileMap.entries()).map(
					([file_path, modPathsSet]) => ({
						file_path,
	...
	const handleSetPriority = async (modPath: string, priority: number) => {
		if (gameRunning && !bypassGameRunningLock) {
			alert.warning("Game Running", "Cannot change priority while game is running.");
			return;
		}
		try {
			...
			if (panels.clash) {
				if (clashScopeModPath.current) {
					...
					const scopeMod = freshMods.find(
						(m: ModRecord) => getCleanBase(m.path) === scopeBase,
					);
					if (scopeMod) {
	...
	const handleSetParallelProcessing = async (enabled: boolean) => {
		try {
			...
			setStatus(
				enabled
					? "Parallel processing set to Boost mode"
					: "Parallel processing set to Normal mode",
			);
		} catch (error) {
	...
	const handleBulkToggle = async (enable: boolean) => {
		if (selectedMods.size === 0 || (gameRunning && !bypassGameRunningLock)) {
			if (gameRunning && !bypassGameRunningLock)
				alert.warning("Game Running", "Cannot toggle mods while game is running.");
			return;
		}
		...
		try {
			...
			const normalizeToggleKey = (path: string) =>
				path.replace(/\.(pak|bak_bento|pak_disabled)$/i, "");
			let count = 0;
			...
			if (selectedMod) {
				const updatedSelected = refreshedByToggleKey.get(
					normalizeToggleKey(selectedMod.path),
				);
				if (updatedSelected) {
	...
	const handleInitiateUpdate = async (mod: ModRecord | null) => {
		...
		try {
			...
			if (selected) {
				...
				console.debug("[UpdateMod] Update modal opened", {
					modPath: mod.path,
	...
	const handleConfirmUpdate = async (preserveName: boolean, obfuscate: boolean) => {
		...
		try {
			...
			console.debug(
				"[UpdateMod] Completed update flow; relying on backend named success toast",
			);
		} catch (e) {
	...
	useEffect(() => {
		...
		const unlistenToast = listen("toast_notification", (event: any) => {
			...
			const toastType =
				typeof type === "string" && type in showAlertByType
	...
	useEffect(() => {
		...
		const unlistenUpdateReady = listen("update_ready_to_apply", () => {
			console.debug(
				"[Updates] Backend reported update ready; waiting for auto-exit/app shutdown",
			);
		});
	...
	const loadInitialData = async () => {
		try {
			...
			if (lastSeen && lastSeen !== ver) {
				console.debug("[Changelog] Version change detected", {
					lastSeen,
					currentVersion: ver,
					normalizedVersion,
				});
				try {
					...
					for (const tag of tagCandidates) {
						const tagUrl = `https://api.github.com/repos/Kuusouu/BentoMod/releases/tags/${encodeURIComponent(tag)}`;
						console.debug("[Changelog] Trying tag endpoint", { tag, tagUrl });
						const tagRes = await fetch(tagUrl, { headers });
						console.debug("[Changelog] Tag endpoint response", {
							tag,
					...
					if (!releaseBody) {
						const latestUrl =
							"https://api.github.com/repos/Kuusouu/BentoMod/releases/latest";
						console.debug("[Changelog] Falling back to latest release endpoint", {
							latestUrl,
						});
						const latestRes = await fetch(latestUrl, { headers });
						console.debug("[Changelog] Latest release response", {
							status: latestRes.status,
							ok: latestRes.ok,
						});
						if (latestRes.ok) {
							const latestRelease = await latestRes.json();
							const latestTagNormalized = String(
							...
							if (latestTagNormalized === normalizedVersion && latestRelease?.body) {
								...
								console.debug(
									"[Changelog] Changelog found via latest release fallback",
								);
							}
						}
					}
	...
	const preloadModDetails = async (modList: ModRecord[]) => {
		...
		try {
			...
			const pathsToFetch = modList.map((m) => m.path).filter((p) => !existing[p]);
	...
	const handleDeleteMod = async (modPath: string) => {
		...
		try {
			// Strip .bak_bento extension to get base path for proper deletion of all associated files
			const basePath = modPath.replace(/\.bak_bento$/i, ".pak");
	...
	const handleToggleMod = async (modPath: string) => {
		...
		try {
			...
			// The path changes from .pak to .bak_bento or vice versa
			const baseName = modPath.replace(/\.(pak|bak_bento)$/i, "");
			...
			if (selectedMod && selectedMod.path === modPath) {
				const updatedMod = refreshedMods.find(
					(m) => m.path.replace(/\.(pak|bak_bento)$/i, "") === baseName,
				);
				if (updatedMod) {
	...
	const handleNewFolderConfirm = async (folderName: string) => {
		...
		if (isMoveAfterCreate && gameRunning && !bypassGameRunningLock) {
			...
			alert.warning("Game Running", "Cannot move folders while game is running.");
			return;
		}
			...
			(async () => {
				try {
					...
					if (isMoveAfterCreate && moveFolderId) {
						const newId = (await invoke("move_folder", {
							id: moveFolderId,
							newParentId: finalFolderName,
						})) as string;
	...
	const handleRenameFolderConfirm = async (newName: string) => {
		...
		try {
			const newId = (await invoke("rename_folder", {
				id: folderId,
				newName: newName,
			})) as any;
			if (selectedFolderId === folderId) {
	...
	const handleToggleModSelection = (mod: ModRecord, e?: MouseEvent | React.MouseEvent) => {
		...
		if (
			e?.shiftKey &&
			e?.ctrlKey &&
			lastSelectedModIndex.current !== null &&
			clickedIndex !== -1
		) {
	...
	const handleAssignToFolder = async (folderId: string | null) => {
		if (gameRunning && !bypassGameRunningLock) {
			alert.warning("Game Running", "Cannot move mods while game is running.");
			return;
		}
	...
	const baseFilteredMods = mods.filter((mod) => {
		...
		if (hasCharFilter || hasCatFilter) {
			...
			if (hasCharFilter) {
				const isMulti =
					typeof d.mod_type === "string" && d.mod_type.startsWith("Multiple Heroes");
	...
	const filteredMods = baseFilteredMods
		.filter((mod) => {
			...
			// Arrow navigation
				...
				if (viewMode === "list" || viewMode === "list-compact") {
					...
					if (key === "arrowup") newIndex = Math.max(0, currentIndex - 1);
						...
						newIndex = Math.min(filteredMods.length - 1, currentIndex + 1);
				} else {
					...
					if (key === "arrowup") newIndex = Math.max(0, currentIndex - itemsPerRow);
						...
						newIndex = Math.min(filteredMods.length - 1, currentIndex + itemsPerRow);
	...
	const saveAllSettings = async (overrides: Partial<AppSettings>) => {
		...
		const newAccentColor =
			overrides.accentColor !== undefined ? overrides.accentColor : accentColor;
		const newViewMode = (
			overrides.viewMode !== undefined ? overrides.viewMode : viewMode
		) as ViewMode;
		const newHideSuffix =
			overrides.hideSuffix !== undefined ? overrides.hideSuffix : hideSuffix;
		const newAutoOpenDetails =
			overrides.autoOpenDetails !== undefined ? overrides.autoOpenDetails : autoOpenDetails;
		const newShowHeroIcons =
			overrides.showHeroIcons !== undefined ? overrides.showHeroIcons : showHeroIcons;
		const newShowHeroBg =
			overrides.showHeroBg !== undefined ? overrides.showHeroBg : showHeroBg;
		const newShowModType =
			overrides.showModType !== undefined ? overrides.showModType : showModType;
		const newShowExperimental =
			overrides.showExperimental !== undefined
				? overrides.showExperimental
				: showExperimental;

		const newParallelProcessing =
			overrides.parallelProcessing !== undefined
				? overrides.parallelProcessing
				: parallelProcessing;
		const newAutoCheckUpdates =
			overrides.autoCheckUpdates !== undefined
				? overrides.autoCheckUpdates
				: autoCheckUpdates;
		const newHoldToDelete =
			overrides.holdToDelete !== undefined ? overrides.holdToDelete : holdToDelete;
		const newShowSubfolderMods =
			overrides.showSubfolderMods !== undefined
				? overrides.showSubfolderMods
				: showSubfolderMods;
		const newBypassGameRunningLock =
			overrides.bypassGameRunningLock !== undefined
				? overrides.bypassGameRunningLock
				: bypassGameRunningLock;
		const newLauncherType =
			overrides.launcherType !== undefined ? overrides.launcherType : launcherType;

		// 2. Map newAccentColor from hex to the preset name if it is a hex value
		const accentName =
			Object.keys(ACCENT_COLORS_MAP).find(
				(key) => ACCENT_COLORS_MAP[key] === newAccentColor,
			) || newAccentColor;
		...
		if (overrides.accentColor !== undefined) {
			...
			const palette = AURORA_PALETTES[newAccentColor] || [
				newAccentColor,
				newAccentColor,
				newAccentColor,
				newAccentColor,
		...
		if (overrides.parallelProcessing !== undefined)
			setParallelProcessing(newParallelProcessing);
		...
		if (overrides.bypassGameRunningLock !== undefined)
		...
		if (newShowExperimental === false && newViewMode === "list-compact") {
			setViewMode("list");
			await invoke("save_app_settings", { settings: { ...payload, viewMode: "list" } }).catch(
				console.warn,
			);
		}
	};
	...
	useEffect(() => {
		...
		invoke("get_app_settings")
			.then((settings: any) => {
				...
				const hexAccent =
					ACCENT_COLORS_MAP[settings.accentColor] ||
					(settings.accentColor?.startsWith("#") ? settings.accentColor : "#be1c1c");
				...
				const palette = AURORA_PALETTES[hexAccent] || [
					hexAccent,
					hexAccent,
					hexAccent,
					hexAccent,
				...
				const parsedViewMode: ViewMode =
					...
					settings.viewMode === "list-compact"
						? settings.viewMode
						: "list";
	...
	return (
		<div className="app">
						...
						title={
							promptPathCount > 0
								...
								: newFolderPrompt?.parentId
									? `Create Folder in ${newFolderPrompt.parentId.split("/").pop()}`
									: "Create New Folder"
						}
			...
			<header className="header">
				<div className="header-branding" data-tour="header-branding">
					<div className="header-title-group">
						<h1 className="font-logo">BentoMod</h1>
						...
						style={{
							background: gameRunning
								...
								: launchSuccess
									? "rgba(76, 175, 80, 0.15)"
									: "rgba(74, 158, 255, 0.1)",
							color: gameRunning ? "#ff9800" : launchSuccess ? "#4CAF50" : "#4a9eff",
							border: gameRunning
				...
				<div className="main-panels" onMouseMove={(e) => handleResizeMove(e.nativeEvent)}>
					...
					<motion.div
						...
						transition={
							isResizing
								? { duration: 0 }
								: { type: "tween", ease: "circOut", duration: 0.35 }
						}
					>
								...
								<div className="sidebar-filters-inner">
									...
									{showCharacterFilters && (
										<HeroFilterDropdown
											...
											onToggle={(target: string | string[]) =>
												setSelectedCharacters((prev) => {
													const next = new Set(prev);

													if (Array.isArray(target)) {
														// Bulk toggle logic
														const allSelected = target.every((t) =>
															next.has(t),
														);
														if (allSelected) {
						...
						<div className="center-panel" data-tour="mod-list">
							<div className="center-header">
								...
								<div className="header-actions" data-tour="header-actions">
									...
									<button
										...
										title={
											isRightPanelOpen ? "Collapse Details" : "Expand Details"
										}
									>
										<ViewSidebarIcon
											fontSize="small"
							...
							<div
								className={`bulk-actions-toolbar ${selectedMods.size === 0 ? "inactive" : ""}`}
							>
								<div
								...
								<div className="bulk-controls">
									<div style={{ width: "200px", height: "40px" }}>
										<CustomDropdown
											...
											onChange={(val) => {
												if (!val) return;
												if (val === "root")
													handleAssignToFolder(null); // Handle move to root
									...
									<div style={{ width: "200px", height: "40px" }}>
										<CustomDropdown
											...
											options={allTags.map((t) => {
												const isApplied = mods.some(
													(m) =>
														selectedMods.has(m.path) &&
														toTagArray(m.custom_tags).includes(t),
												);
												return {
													value: t,
													label: t,
													showDelete: isApplied,
												};
											})}
											value=""
											onChange={(tag) => {
												...
												setNewTagPrompt({
													callback: async (tag) => {
														...
														if (trimmed) {
															...
															await invoke("add_tag_to_catalog", {
																tag: trimmed,
															});
									...
									{(() => {
										const selMods = mods.filter((m) =>
											selectedMods.has(m.path),
										);
										const enabledCount = selMods.filter(
											(m) => m.enabled,
										).length;
										...
										return (
											...
											>
												<button
													...
													title={
														allEnabled
															...
															: `Enable ${disabledCount} disabled mod${disabledCount !== 1 ? "s" : ""}`
													}
												>
													<ToggleOnIcon
														fontSize="small"
												...
												<button
													...
													disabled={
														selectedMods.size === 0 || allDisabled
													}
													title={
														allDisabled
															? "All selected mods are already disabled"
															: `Disable ${enabledCount} enabled mod${enabledCount !== 1 ? "s" : ""}`
													}
												>
													<ToggleOffIcon
														fontSize="small"
									...
									<button
										...
										title={
											holdToDelete
												? "Hold 2s to delete selected mods"
												: "Click to delete selected mods"
										}
									>
						...
						{selectedMod ? (
							<div className="mod-details-and-contents">
								...
								<div className="selected-mod-contents">
									...
									<FileTree
										files={
											selectedMod.file_contents ||
											selectedMod.files ||
											selectedMod.file_list ||
											[]
										}
									/>
								</div>
			...
			{contextMenu && (
				<ContextMenu
					...
					onAssignTag={(tag) =>
						contextMenu.mod && handleAddTagToSingleMod(contextMenu.mod.path, tag)
					}
					onNewTag={(callback) => setNewTagPrompt({ callback })}
					onMoveTo={(folderId) =>
						contextMenu.mod && handleMoveSingleMod(contextMenu.mod.path, folderId)
					}
					onMoveFolderTo={(newParentId) =>
						contextMenu.folder && handleMoveFolder(contextMenu.folder.id, newParentId)
					}
					...
					onCheckConflicts={() =>
						contextMenu.mod && handleCheckSingleModClashes(contextMenu.mod)
					}
					onUpdateMod={() => contextMenu.mod && handleInitiateUpdate(contextMenu.mod)}
					onSendToVfxUpdater={() =>
						contextMenu.mod && handleSendToVfxUpdater(contextMenu.mod)
					}
			...
			<OnboardingTour isOpen={showOnboarding} onClose={handleCloseTour} />
		</div>
	);
}
