...
const buildInitialSettings = (mods: ModInput[] = [], currentFolderId?: string): Record<number, ModSetting> => {
  return mods.reduce((acc, mod, idx) => {
    ...
    const effectiveToBento = isHybrid ? true : (!canApplyPatches ? false : (locked ? false : defaultToBento))
            ...
            <div className="install-mod-grid">
              {mods.map((mod, idx) => {
                ...
                const bentoTitle = bentoLocked
                  ? (mod.is_dir ? 'Folder drops cannot be bentoed' : 'Detected loose assets; bento handled automatically')
                      ...
                      <div className="install-mod-card__footer">
                        ...
                        {mod.contains_uassets !== false && (
                          <Switch
                            size="md"
                            color="secondary"
                            checked={modSettings[idx]?.hybrid ? true : (modSettings[idx]?.toBento || false)}
                            onChange={(value) => updateModSetting(idx, 'toBento', value)}
                            isDisabled={bentoLocked || modSettings[idx]?.hybrid}
                            className={`install-toggle bento-toggle ${(bentoLocked || modSettings[idx]?.hybrid) ? 'locked' : ''} ${modSettings[idx]?.toBento ? 'active' : ''}`}
                            title={bentoTitle}
                          >
                            <div className="install-toggle__text">
                              <span className="install-toggle__label">Send to Bento</span>
                              <span className="install-toggle__hint">
                                {bentoLocked ? (mod.is_dir ? 'Not available for folder drops' : 'Loose assets detected') : 'Bentos the pak into IOStore format'}
                              </span>
                            </div>
                          </Switch>
                        )}
