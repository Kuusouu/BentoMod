...
export default function ModDetailsPanel({
	...
	useEffect(() => {
		...
		const loadDetails = async () => {
			...
			if (
				initialDetails &&
				(initialDetails.mod_path === mod.path || !initialDetails.mod_path)
			) {
	...
	const cleanName = suffixMatch
		? nameWithoutExt.slice(0, -suffixMatch[0].length)
		: nameWithoutExt;
						...
						<div className="detail-section">
							...
							>
								{(details.is_iostore ||
									...
									<div style={{ display: "flex", gap: "0" }}>
										{details.is_iostore && (
											<div
												className="iostore-badge"
												style={{
													borderTopRightRadius:
														details.is_hybrid || details.is_encrypted
															? "0"
															: "4px",
										...
										{details.is_hybrid && (
											<div
												...
												style={{
													...
													borderTopLeftRadius: details.is_iostore
														? "0"
														: "4px",
													borderBottomLeftRadius: details.is_iostore
														? "0"
														: "4px",
													borderTopRightRadius: details.is_encrypted
														? "0"
														: "4px",
													borderBottomRightRadius: details.is_encrypted
														? "0"
														: "4px",
													fontWeight: 600,
													fontSize: "0.85rem",
													...
													borderLeft: details.is_iostore
														? "none"
														: "1px solid rgba(76, 175, 80, 0.35)",
												}}
												title="Contains both IO Store assets and Legacy Pak raw assets"
											>
												Hybrid
											</div>
										)}
										{details.is_encrypted && (
											<div
												...
												style={{
													...
													padding: "0.4rem 0.75rem",
													borderTopLeftRadius:
														details.is_iostore || details.is_hybrid
															? "0"
															: "4px",
												...
												title="This mod package is encrypted"
											>
												Encrypted
											</div>
										)}
								...
								{!details.is_iostore &&
									...
									!details.files.some((f) =>
										f.toLowerCase().endsWith(".uasset"),
									) && (
										<div
											className="no-uassets-badge"
											title="This mod contains no UAsset files"
											style={{
												padding: "0.4rem 0.75rem",
							...
							<div className="detail-item">
								...
								<span className="detail-value">
									{formatFileSize(details.total_size ?? 0)}
								</span>
							</div>
							{mod.folder_id && (
								<div className="detail-item">
