...
type HeroFilterItemProps = {
	hero: HeroGroup;
	selectedCharacters: Set<string>;
	onToggle: (value: string | string[]) => void;
};

type HeroFilterDropdownProps = {
...
const groupCharactersByHero = (
	characters: string[],
	modCounts: Record<string, number>,
): HeroGroup[] => {
	const groups: Record<string, HeroGroup> = {};
...
const HeroFilterItem = ({ hero, selectedCharacters, onToggle }: HeroFilterItemProps) => {
	...
	const handleDefaultSkinClick = (e: React.MouseEvent<HTMLButtonElement>) => {
		e.stopPropagation();
		onToggle(hero.baseName);
	};

	const handleSkinClick = (e: React.MouseEvent<HTMLButtonElement>, skinFullName: string) => {
		e.stopPropagation();
		...
		<div className="hero-filter-item">
			<div className="hero-filter-row">
				...
				>
					...
					{totalModCount > 0 && <span className="hero-mod-count">{totalModCount}</span>}
				</button>
			</div>
...
export default function HeroFilterDropdown({
	availableCharacters,
	selectedCharacters,
	modDetails,
	onToggle,
}: HeroFilterDropdownProps) {
	const modCounts = useMemo(() => calculateModCounts(modDetails), [modDetails]);
	const heroGroups = useMemo(
		() => groupCharactersByHero(availableCharacters, modCounts),
		[availableCharacters, modCounts],
	);
	...
	const genericModCount = Object.values(modDetails || {}).filter(
		(d) =>
			!d?.character_name ||
			d?.character_name === "Generic" ||
			d?.character_name === "Unknown",
	).length;
	...
	return (
		<div className="hero-filter-dropdown">
			...
			<div className="hero-filter-special">
				...
				>
					Generic{" "}
					{genericModCount > 0 && <span className="chip-count">{genericModCount}</span>}
				</button>
			</div>
