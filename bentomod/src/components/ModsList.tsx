...
const ModItem = memo(function ModItem({
	...
	const heroImage =
		showHeroIcons || showHeroBg
			? getHeroImage(characterName, characterData, modDetails?.character_id)
			: null;
	...
	const heroesList =
		isMultiHero && modDetails?.files && Array.isArray(modDetails.files)
									...
									{heroesList.map((name) => (
										<span key={name} className="tag hero-tag">
											{getHeroImage(name, characterData) && (
												<img
													src={getHeroImage(name, characterData)}
													alt=""
												/>
											)}
											{name}
			...
			tags.length > 0 ? (
				<div className="mod-tags-row">
					...
					{showModType && category && isCardView && (
						<span
							className={`mod-type-badge category-badge ${category.toLowerCase().replace(/\s+/g, "-")}-badge`}
						>
							{category}
						</span>
					)}
