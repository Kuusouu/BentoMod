...
export function AlertProvider({
	...
	const showAlert = useCallback((alertConfig: AlertOptions) => {
		...
		const toast: Toast = {
			...
			...alertConfig,
			duration:
				typeof alertConfig.duration === "number"
	...
	const promise = useCallback(
		<T = any>(promiseOrFn: Promise<T> | (() => Promise<T>), options: AlertOptions = {}) => {
			const {
			...
			thePromise
				.then((result) => {
					...
					const successOptions =
						typeof successConfig === "function" ? successConfig(result) : successConfig;
					updateToast(id, {
						...successOptions,
						color: "success",
						isLoading: false,
				...
				.catch((err) => {
					...
					const errorOptions =
						typeof errorConfig === "function" ? errorConfig(err) : errorConfig;
					updateToast(id, {
						...errorOptions,
						color: "danger",
						isLoading: false,
...
const ToastItem = memo(function ToastItem({
	...
	return (
		...
		>
			{!hideIcon && <div className="toast-icon">{getIcon()}</div>}
			...
			{((shouldAutoDismiss && duration > 0) || typeof progressValue === "number") && (
				<div className="toast-progress-track">
					<div
						...
						style={{
							transform: `scaleX(${(typeof progressValue === "number" ? progressValue : progress) / 100})`,
						}}
					/>
				</div>
			)}
