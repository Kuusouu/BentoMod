...
export function usePipeline({
	...
	const runPipeline = useCallback(async () => {
		...
		try {
			...
			addLog(
				`✓ Found ${updatableAssets.length} processable (blueprint/material_instance) and ${nonUpdatableAssets.length} bypass assets`,
				"success",
			);
			console.debug("[VFX] Asset classification", {
				updatable: updatableAssets.length,
				nonUpdatable: nonUpdatableAssets.length,
			});
			...
			const parseResults = await parallelMapWithLimit(
				modJsonFiles,
				parseConcurrency,
			...
			if (allColors.length === 0) {
				addLog(
					"No color parameters found - pipeline will create passthrough mod",
					"warning",
				);
			}
			...
			const applyResults = await parallelMapWithLimit(
				vanillaJsonFiles,
				applyConcurrency,
				async (jsonPath) => {
					if (runIdRef.current !== currentRunId) throw new Error("Pipeline cancelled");
					try {
						...
						const matchingColors = allColors.filter(
							(c) =>
								...
								c.relativePath.replace(".json", "") ===
									relativePath.replace(".json", "") ||
								c.relativePath.split(/[\\/]/).pop() ===
						...
						if (matchingColors.length > 0) {
							...
							for (const color of matchingColors) {
								try {
									...
									if (applied) {
										...
										fileWarnings.push(
											`Could not apply ${color.paramName}: path not found`,
										);
									}
								} catch (e) {
							...
							if (appliedToFile > 0) {
								...
								const editedPath = jsonPath.replace(
									tempDirs.vanillaJson,
									tempDirs.editedJson,
								);
						...
						return {
							appliedToFile: 0,
							fileWarnings: [`Failed to process: ${jsonPath}`],
						};
					}
				},
			);
