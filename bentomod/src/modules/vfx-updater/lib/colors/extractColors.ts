...
export function findColorsRecursive(
	...
	const isLinearColorProperty =
		colorNames.includes(currentObject.Name) && currentObject.StructType === "LinearColor";
	const isSlateColorProperty =
		colorNames.includes(currentObject.Name) && currentObject.StructType === "SlateColor";
			...
			if (paramName) {
				...
				if (linearColor) {
					...
					const path: (string | number)[] = [
						...
						0,
						"Data",
						json.Exports[0].Data.findIndex(
							(p: any) => p.Name === "VectorParameterValues",
						),
						"Value",
						paramIndex,
						"Value",
					...
					console.debug("[VFX] Extracted VectorParameter color", {
						paramName,
						path,
						rgba: sanitizedRgba,
					});
