...
export default function VfxUpdaterPanel() {
			...
			try {
				...
				if (rxModsPath) {
					const lastSlash = Math.max(
						rxModsPath.lastIndexOf("/"),
						rxModsPath.lastIndexOf("\\"),
					);
					const paksPath =
						lastSlash > 0 ? rxModsPath.substring(0, lastSlash) : rxModsPath;
	...
	const runUsmapCheck = useCallback(
		async (force: boolean) => {
			...
			try {
				...
				if (result.updated && result.localPath && !result.skipped) {
					...
					addLog(
						`USMAP ${force ? "fetched" : "auto-updated"}: ${result.filename ?? "(latest)"}`,
						"success",
					);
					...
					if (force && result.localPath) {
						setUsmapPath(result.localPath);
						addLog(
							`USMAP replaced with latest: ${result.filename ?? "(latest)"}`,
							"success",
						);
					} else {
					...
					const details = [
						...
						.join(" | ");
					addLog(
						`USMAP up to date${details ? ` (${details})` : ""}`,
						force ? "success" : "info",
					);
		...
		<div className="vfx-panel">
			<header className="vfx-header">
				<h1 className="font-logo">
					Bento <AuroraText className="font-logo">VFX</AuroraText>
				</h1>
			</header>
				...
				{viewMode === "input" && (
					...
					>
						...
						<ShineBorder
							...
							shineColor={
								isStartDisabled
									? ["var(--panel-border, #333)", "transparent"]
									: [
											"var(--accent-primary)",
											"color-mix(in srgb, var(--accent-primary), transparent 50%)",
										]
							}
						>
							<button
						...
						<div className="vfx-progress-top-panel">
							...
							<div className="vfx-segmented-track">
								{PIPELINE_STEPS.map((step) => (
									<div
										key={step.id}
										className={`vfx-segment ${clampedStep > step.id ? "completed" : clampedStep === step.id ? "active" : ""}`}
									/>
								))}
							</div>
							<div className="vfx-step-status">
								{stepStatus?.message || "Running Pipeline..."}
							</div>
						</div>
						...
						<div className="vfx-dial-panel">
							<div className="vfx-dial-container">
								...
								<div className="vfx-dial-text">
									<span className="vfx-dial-percent">
										<motion.span>{animatedPercentageText}</motion.span>%
									</span>
									<span className="vfx-dial-fraction">
										{clampedStep} / {TOTAL_STEPS}
									</span>
								</div>
							</div>
						</div>
			...
			<div className="vfx-logs">
				{logs.length > 0 && (
					...
					>
						{copyFeedback ? (
							<span style={{ fontSize: "10px", marginRight: "4px" }}>Copied!</span>
						) : null}
						<BiCopyAlt size={16} />
					</button>
				)}
